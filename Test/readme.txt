...
## Command rule (STRICT)
For any PR review request, export first.

Normal review:
- `review pr <PR_URL>`
- `review pull request <PR_URL>`
- equivalent requests without full-file context
...
- equivalent requests asking for full-file context
...
- low-risk files may be `lightweight reviewed`, but still must be inspected
...
A `material changed hunk` is one that may affect:
- runtime behavior
- control flow
- validation
- permissions
