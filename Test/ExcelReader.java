...
| DEAL_BRNCH_CDE | FOS.NMEMO_DEAL.DEAL_BRANCH_CODE | If region HK then 02. If region SG and asset type FX then 02 when local currency equals NT.SETTLEMENT_CURRENCY or NT.NMEMO_CURRENCY_CODE. If region SG and asset type is not FX then 02 when local currency equals NT.SETTLEMENT_CURRENCY. Otherwise 03. | Branch or COBR rule. Broker deal uses template and local currency logic. |
...
| ASSET_CLASS | Common asset-class mapping from NT.ASSET_TYPE | Same | TRS normally outputs MX when NT.ASSET_TYPE is MX. |
...
| BULLION_FLAG | If top FX underlying exists, call INTERFACE.FN_GET_FTP_BULLION_FLAG using the two currencies in UNDERLYING_ISSUE_CODE. Otherwise map NT.NMEMO_CURRENCY_CODE XAU = 1, XAG = 2, XPT = 3, XPD = 4, else 0. | Same | Bullion indicator for FX or precious-metal structures. |
| TOTAL_RETURN_SWAP_FLAG | Y when NT.NMEMO_CODE in ('TR4','TR5'), NT.ASSET_TYPE = 'MX', and primary underlying class in EQ or NX. Otherwise N. | Same | Main TRS identifier in this target table. |
...
| OPT_TYPE | NT.OPTION_TYPE when it is C or P, else empty string | Same | Option type when applicable. |
| OPT_STYL_CDE | E when NT.NMEMO_PAYOFF_STYLE = 'EURO', else A | Same | Option style when applicable. |
| TRADE_DT | FOS.NMEMO_DEAL.NMEMO_TRADE_DATE | FOS.NMEMO_BROKER_DEAL.NMEMO_TRADE_DATE | Trade date. |
| MTUR_DT | FOS.NMEMO_DEAL.NMEMO_DELIVERY_DATE | NT.NMEMO_DELIVERY_DATE | Maturity or delivery date. |
| VALUE_DT | FOS.NMEMO_DEAL.NMEMO_VALUE_DATE | FOS.NMEMO_BROKER_DEAL.NMEMO_VALUE_DATE | Value date. |
| FIXING_DT | NT.NMEMO_EXPIRY_DATE | NT.NMEMO_EXPIRY_DATE | Final fixing or expiry date. |
...
| NOTIONAL_CCY | If ND.NMEMO_CURRENCY_CODE = 'ASSET' then ND.SETTLEMENT_CURRENCY_CODE, else ND.NMEMO_CURRENCY_CODE. | If NT.NMEMO_CURRENCY_CODE = 'ASSET' then NT.SETTLEMENT_CURRENCY, else NT.NMEMO_CURRENCY_CODE. | Reported notional currency. |
| NOTIONAL_AMT | Initial ND.SETTLEMENT_NOTIONAL for asset currency, else ND.NMEMO_NOMINAL. Final value is multiplied by leverage factor. | Initial 0 for asset currency, else BROKER_ND.NMEMO_NOMINAL. If asset currency, later replaced by sum of linked client NOTIONAL_AMT before leverage is applied. Final value is multiplied by leverage factor. | Gross notional. |
...
| PREM_CCY | FOS.NMEMO_DEAL.SETTLE_CURRENCY_CODE | NT.SETTLEMENT_CURRENCY | Premium or consideration currency. |
...
| PREM_PAY_RECEIVE_FLAG | P when ND.CONSIDERATION_AMOUNT > 0, else R | R when BROKER_ND.CONSIDERATION_AMOUNT > 0, else P | Bank-view premium direction. |
...
| CUR_MTM_AMT | Initially 0. Later MPP.MARKET_VALUE * -1, unless FINAL_FIXING_FLAG = 'Y' then 0. | Initially 0. Later negative sum of linked client CUR_MTM_AMT. | Current MTM in bank view. |
| PREVIOUS_MTM_AMT | Initially 0. Later MPPLD.MARKET_VALUE * -1, default 0. | Initially 0. Later negative sum of linked client PREVIOUS_MTM_AMT. | Previous-day MTM in bank view. |
...
| UNDL_ISIN_CDE | Initial NT.NMEMO_ISIN_CODE on client side. Later single EQ or NX update also uses NT.NMEMO_ISIN_CODE. | Initially empty. Later single EQ or NX update uses NT.NMEMO_ISIN_CODE. | ISIN code. |
...
| GUARANTOR_NUM | NTAD.GUARANTOR_NUMBER | NTAD.GUARANTOR_NUMBER | Guarantor identifier. |
| ISSUER_NUM | NTAD.ISSUER_NUMBER | NTAD.ISSUER_NUMBER | Issuer identifier. |
...
| ADVANCE_PURPOSE_CODE | If NT.ASSET_TYPE = 'MX', use primary underlying class, with NX normalized to EQ; otherwise empty string. Null cleanup sets empty string. | Same | Underlying purpose or exposure class for MX products. |
| TEMPLATE_ISSUE_CODE | NT.NMEMO_ISSUE_CODE | FOS.NMEMO_BROKER_DEAL.NMEMO_ISSUE_CODE | Template identity used by later joins and updates. |
| FINAL_FIXING_FLAG | Initial N. Later Y when latest eligible payment has remaining settlement notional 0 and event type ER or M, or fixing date is greater than or equal to NT.NMEMO_EXPIRY_DATE. | Same using FOS.NMEMO_BANK_PAYMENT. | Final maturity or early redemption indicator. |
...
### 4.2 GM_COMMON_DEAL Post-Insert Logic

| Step | Client side logic | Broker side logic | Result |
|---|---|---|---|
| Final fixing flag | Uses latest eligible FOS.NMEMO_PAYMENT by max FIXING_DATE per deal. Sets FINAL_FIXING_FLAG = 'Y' when remaining settlement notional is 0 and event is ER or M, or fixing date reaches template expiry. | Same using FOS.NMEMO_BANK_PAYMENT. | Matured or early-redeemed trades have final flag Y. |
| Open deal link | Matches open and close FOS.NMEMO_DEAL rows under same customer, sub account and deal number but opposite OPT_POSN_TYPE. | Uses FOS.NMEMO_BANK_ORDER plus FOS.NMEMO_ORDER. The fixed SQL contains separate broker merges for open rows and close rows. | OPEN_DEAL_NUM and OPEN_DEAL_SB_NUM are populated for lifecycle linkage. |
| Previous outstanding notional | Uses GM_COMMON_DEAL_LASTDAY when matched. For new BUS_DT rows missing in lastday, uses NOTIONAL_AMT * leverage factor. | Same. | Previous-day exposure baseline. |
| Current outstanding notional | Uses payment remaining notional. Asset currency uses REMAINING_SETTLEMENT_NOTIONAL. Non-asset currency uses REMAINING_NMEMO_NOTIONAL. The SQL groups by deal and uses MIN remaining amount with MAX fixing date. | Same with bank payment. | Current remaining notional before final leverage multiplication. |
| Leverage | Multiplies OUTSTANDING_NOTIONAL_AMT and NOTIONAL_AMT by leverage factor. | Same. | Final notional values are leverage-adjusted. |
| MTM | Uses INTERFACE.MEMO_PRODUCT_POSITION for current MTM and MEMO_PRODUCT_POSITION_LASTDAY for previous MTM. Client MTM is multiplied by -1. Final-fixed rows get current MTM 0. | Derived as the negative aggregate of linked client MTM rows. | Bank-view MTM is stored on both sides. |
| Knockout date | Uses FOS.NMEMO_PAYMENT.FIXING_DATE where FIXING_EVENT_TYPE = 'ER'. | Same using FOS.NMEMO_BANK_PAYMENT. | Early redemption date. |
| Deposit linked fields | If FOS.NMEMO_TEMPLATE_DEPO exists, sets DEPOSIT_LINKED_FLAG = 'Y' and calculates TOTAL_INT_AMT from DEPO_STYLE and DEPO_LEVEL. | Same. | Deposit-linked structures carry interest amount. |
| Underlying details | For templates with exactly one active underlying and asset type EQ or NX, updates underlying issue, currency, Reuters code, ISIN and country. | Same. | Single-name equity or index products get detailed underlying attributes. |
| Cleanup | Null UNDL_ISSUE_CRNCY_CDE becomes empty string. Blank MTM_CCY falls back to PREM_CCY. Close deals copy MTM_CCY and OUTSTANDING_NOTIONAL_AMT from matched open deal. Null ADVANCE_PURPOSE_CODE becomes empty. Null LINK_DEAL_NUM/SB for supported asset classes becomes 0. | Same target-level cleanup. | Final target values are normalized for downstream consumers. |
...
| Target field | Client side mapping | Broker side mapping | Business description |
|---|---|---|---|
| ACTV_FLAG | ND.ACTIVE_FLAG | BROKER_ND.ACTIVE_FLAG | Source active flag. |
| DEAL_NUM | ND.DEAL_NUMBER | BROKER_ND.DEAL_NUMBER | Deal number. |
| DEAL_SB_NUM | ND.DEAL_SUB_NUMBER | Constant 0 | Deal sub number. |
| DEAL_COMP_CDE | ND.DEAL_COMPANY_CODE | Region SG = 68, HK = 63 | Company code. |
| DEAL_BRNCH_CDE | ND.DEAL_BRANCH_CODE | HK = 02. SG = 02 when local currency equals NT.SETTLEMENT_CURRENCY, else 03. | Branch code for extension row. |
| DEAL_TRAN_TYPE | ND.DEAL_TRANSACTION_TYPE O maps to S, otherwise P | BROKER_ND.DEAL_TRANSACTION_TYPE O maps to P, otherwise S | Bank-view direction. |
| CUST_NUM | ND.CUSTOMER_NUMBER | BROKER_ND.NMEMO_BROKER_NUMBER | Client customer or broker counterparty. |
| SB_ACCT_NUM | ND.SUB_ACCOUNT_NUMBER | Constant '0001' | Sub account. |
| DEAL_TYPE | Constant MM | Constant MM | Generic Memo deal type. |
| PROD_TYPE | Constant MM | Constant MM | Generic Memo product type. |
| ASSET_CLASS | Common asset-class mapping from NT.ASSET_TYPE | Same | Because this procedure filters NT.ASSET_TYPE = 'IR', output is normally IR. |
| GROS_SETL_FLAG | Constant '0' | Constant '0' | Gross settlement flag. |
| PAY_INT_RATE_CDE | Pay leg underlying issue code. If length > 4, use first 2 chars plus last 2 chars. | Same logic, but broker pay leg is selected with LEG_WEIGHT < 0. | Pay leg rate code. |
| RECV_INT_RATE_CDE | Receive leg underlying issue code. If length > 4, use first 2 chars plus last 2 chars. | Same logic, but broker receive leg is selected with LEG_WEIGHT >= 0. | Receive leg rate code. |
| PAY_UNDL_CCY_CDE | Pay leg underlying currency. | Same. | Pay leg underlying currency. |
| RECV_UNDL_CCY_CDE | Receive leg underlying currency. | Same. | Receive leg underlying currency. |
...
| PAY_MRGN_INT_RATE | Pay leg UNDERLYING_INITIAL_SPOT when pay underlying issue code is FIX, else 0. | Same. | Fixed margin rate for pay leg. |
| RECV_MRGN_INT_RATE | Receive leg UNDERLYING_INITIAL_SPOT when receive underlying issue code is FIX, else 0. | Same. | Fixed margin rate for receive leg. |
...
| PAY_NXT_RT_FIXING_DT | COALESCE(pay NEXT_FIXING_DATE from FOS.NMEMO_FIXING_DATE_DETAIL_EXT, NT.NMEMO_EXPIRY_DATE). | Same. | Next pay-leg rate fixing date. |
| RECV_NXT_RT_FIXING_DT | COALESCE(receive NEXT_FIXING_DATE from FOS.NMEMO_FIXING_DATE_DETAIL_EXT, NT.NMEMO_EXPIRY_DATE). | Same. | Next receive-leg rate fixing date. |
...
| PAY_NEXT_SETL_DT | COALESCE(pay NEXT_SETTLE_DATE, NT.NMEMO_EXPIRY_DATE). | Same. | Next pay-leg settlement date. |
| RECV_NEXT_SETL_DT | COALESCE(receive NEXT_SETTLE_DATE, NT.NMEMO_EXPIRY_DATE). | Same. | Next receive-leg settlement date. |
| SCH_START_DATE | COALESCE(pay LAST_FIXING_DATE, receive LAST_FIXING_DATE, ND.NMEMO_VALUE_DATE). | COALESCE(pay LAST_FIXING_DATE, receive LAST_FIXING_DATE, BROKER_ND.NMEMO_VALUE_DATE). | Schedule start date. |
| SCH_END_DATE | COALESCE(pay NEXT_FIXING_DATE, receive NEXT_FIXING_DATE, NT.NMEMO_EXPIRY_DATE). | Same. | Schedule end date. |
...
| DEAL_NUM | PAYMENT.NMEMO_DEAL_NUMBER | PAYMENT.NMEMO_BANK_DEAL_NUMBER | Fixing deal number used by target. |
| DEAL_SB_NUM | PAYMENT.NMEMO_DEAL_SUB_NUMBER | PAYMENT.NMEMO_BANK_DEAL_SUB_NUMBER | Fixing deal sub number. |
| DEAL_COMP_CDE | Region SG = 68, HK = 63 | Same | Company code is region-derived for fixing rows on both sides. |
| DEAL_BRNCH_CDE | HK = 02. SG = 02 when local currency equals PAYMENT.PAY_CURRENCY_CODE or PAYMENT.RECEIVE_CURRENCY_CODE, else 03. | Same rule using FOS.NMEMO_BANK_PAYMENT pay/receive currencies. | Latest fixed SQL derives fixing branch from actual payment currencies, not template settlement currency. |
...
| CUST_NUM | PAYMENT.CUSTOMER_NUMBER | PAYMENT.BROKER_NUMBER | Client customer or broker counterparty. |
| SB_ACCT_NUM | PAYMENT.SUB_ACCOUNT_NUMBER | Constant '0001' | Sub account. |
| DEAL_TYPE | Constant MM | Constant MM | Generic Memo deal type. |
| PROD_TYPE | Constant MM | Constant MM | Generic Memo product type. |
| ASSET_CLASS | Common asset-class mapping from NMEMO_TEMPLATE.ASSET_TYPE | Same | Asset class for the original deal template. |
| BANK_CUST_FLAG | C | B | Target side. |
| FIXING_EVENT_TYPE | PAYMENT.FIXING_EVENT_TYPE | PAYMENT.FIXING_EVENT_TYPE | Event type such as ER, M, PD or other source values. |
| START_DT | PAYMENT.PERIOD_START_DATE | PAYMENT.PERIOD_START_DATE | Cashflow period start date. |
| END_DT | PAYMENT.FIXING_DATE | PAYMENT.FIXING_DATE | Fixing date or cashflow period end. |
| CASH_SETL_FLAG | Y when NMEMO_TEMPLATE.NMEMO_SETTLEMENT_TYPE = 'CASH', else N | Same | Cash settlement indicator. |
| INT_RATE | COALESCE(IR FIX underlying UNDERLYING_INITIAL_SPOT, 0) | Same | Interest rate from the IR FIX underlying, if present. |
| PAY_RECEIVE_FLAG | If PAY_AMOUNT > 0 and RECEIVE_AMOUNT is null then R. If RECEIVE_AMOUNT > 0 and PAY_AMOUNT is null then P. Else empty. | If PAY_AMOUNT > 0 and RECEIVE_AMOUNT is null then P. If RECEIVE_AMOUNT > 0 and PAY_AMOUNT is null then R. Else empty. | Bank-view pay or receive flag. Client and broker rules intentionally differ. |
| SETL_DT | PAYMENT.SETTLE_DATE | PAYMENT.SETTLE_DATE | Settlement date. |
| SETL_CCY | When FX_DEAL_NUMBER is null, COALESCE(PAY_CURRENCY_CODE, RECEIVE_CURRENCY_CODE). | Same | Direct settlement currency when no linked FX deal exists. |
| SETL_AMT | When FX_DEAL_NUMBER is null, COALESCE(PAY_AMOUNT, RECEIVE_AMOUNT, 0); else 0. | Same | Direct settlement amount. FX settlement rows carry buy/sell fields instead. |
| BUY_CCY | When FX_DEAL_NUMBER is not null, RECEIVE_CURRENCY_CODE. | Same | FX buy currency. |
| BUY_AMT | When FX_DEAL_NUMBER is not null, COALESCE(RECEIVE_AMOUNT, 0). | Same | FX buy amount. |
| MRK_RECV_SETL_DT | PAYMENT.RECEIVE_MARK_SETTLE_DATE | CLIENT_PAYMENT.RECEIVE_MARK_SETTLE_DATE | Market receive settlement date. Broker side uses linked client payment. |
| SELL_CCY | When FX_DEAL_NUMBER is not null, PAY_CURRENCY_CODE. | Same | FX sell currency. |
| SELL_AMT | When FX_DEAL_NUMBER is not null, COALESCE(PAY_AMOUNT, 0). | Same | FX sell amount. |
| MRK_PAY_SETL_DT | PAYMENT.PAY_MARK_SETTLE_DATE | CLIENT_PAYMENT.PAY_MARK_SETTLE_DATE | Market pay settlement date. Broker side uses linked client payment. |
| CROS_DEAL_NUM | PAYMENT.DEAL_NUMBER | PAYMENT.BANK_DEAL_NUMBER | Original cross deal number. |
| CROS_DEAL_SB_NUM | PAYMENT.DEAL_SUB_NUMBER | PAYMENT.BANK_DEAL_SUB_NUMBER | Original cross deal sub number. |
| LINK_DEAL_TYPE | 51 when PAYMENT.FX_DEAL_NUMBER exists. Else 76 when PAYMENT.SHARE_DEAL_NUMBER exists. Else 04 when PAY_DEAL_NUMBER or RECEIVE_DEAL_NUMBER exists. Else empty. | 51 when bank FX_DEAL_NUMBER exists. Else 76 when CLIENT_PAYMENT.SHARE_DEAL_NUMBER exists. Else 04 when CLIENT_PAYMENT.PAY_DEAL_NUMBER or RECEIVE_DEAL_NUMBER exists. Else empty. | Downstream linked settlement type: FX, share, or cash/pay-receive. |
| LINK_DEAL_NUMBER | COALESCE(FX_DEAL_NUMBER, SHARE_DEAL_NUMBER, PAY_DEAL_NUMBER, RECEIVE_DEAL_NUMBER). | COALESCE(bank FX_DEAL_NUMBER, CLIENT_PAYMENT.SHARE_DEAL_NUMBER). | Broker SQL does not coalesce pay/receive deal number even when LINK_DEAL_TYPE = 04. This is a confirmed mapping gap to review. |
| LINK_DEAL_SUB_NUMBER | COALESCE(FX_DEAL_SUB_NUMBER, SHARE_DEAL_SUB_NUMBER, PAY_DEAL_SUB_NUMBER, RECEIVE_DEAL_SUB_NUMBER). | COALESCE(bank FX_DEAL_SUB_NUMBER, CLIENT_PAYMENT.SHARE_DEAL_SUB_NUMBER). | Same broker gap as LINK_DEAL_NUMBER. |
| CREAT_TS | PAYMENT.CREATE_TIMESTAMP | PAYMENT.CREATE_TIMESTAMP | Audit create timestamp. |
| RVRSE_TS | PAYMENT.REVERSE_TIMESTAMP | PAYMENT.REVERSE_TIMESTAMP | Audit reverse timestamp. |
| UPDT_TS | PAYMENT.UPDATE_TIMESTAMP | PAYMENT.UPDATE_TIMESTAMP | Audit update timestamp. |
| PAY_OUR_NOSTRO_NUMBER | Not inserted by client side SQL. | PAYMENT.PAY_OUR_NOSTRO_NUMBER | Broker-side pay nostro account. |
| RECEIVE_OUR_NOSTRO_NUMBER | Not inserted by client side SQL. | PAYMENT.RECEIVE_OUR_NOSTRO_NUMBER | Broker-side receive nostro account. |
| PAY_THEIR_NOSTRO_NUMBER | Not inserted by client side SQL. | PAYMENT.PAY_THEIR_NOSTRO_NUMBER | Counterparty pay nostro account. |
| RECEIVE_THEIR_NOSTRO_NUMBER | Not inserted by client side SQL. | PAYMENT.RECEIVE_THEIR_NOSTRO_NUMBER | Counterparty receive nostro account. |
...
| Broker fixing LINK_DEAL_NUMBER for type 04 | Open issue | Broker LINK_DEAL_TYPE can be 04 based on CLIENT_PAYMENT.PAY_DEAL_NUMBER or RECEIVE_DEAL_NUMBER, but LINK_DEAL_NUMBER only coalesces FX and share deal number. | Confirm whether pay/receive deal numbers should also be coalesced on broker side. |
